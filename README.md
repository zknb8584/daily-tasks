...
- **角色扮演（长期聊天框）**：导入 `.txt`/JSON 角色卡（固定 `[核心] [背景] [爱好] [说话风格] [关系] [扩展] [记忆] [当前情绪] [好感度] [系统提示] [历史后置指令] [世界书]` 段名），兼容 Character Card V2/TavernAI 的 `system_prompt`、`post_history_instructions`、`character_book`；开始对话前可选初始身份和好感度；每个角色一个永久聊天框；按需加载角色卡段省 token；世界书会按用户消息里的关键词自动加载；角色会从 `[爱好]` 等设定里主动发起话题；好感度/情绪/记忆自动更新；「一定要记得…」会写入重要记忆；可重置关系、删除角色卡。
- **AI 群聊**：AI 中心新增「我的群聊」，可选 2~4 个角色创建群聊；群聊消息持久保存，自动 1~2 个角色接话，支持用 `@角色名` 指定回复者；角色在群聊中产生的「群聊记忆」会带回私聊，角色独立的好感度/情绪/记忆仍然全局保留。
