...
- **AI 中心三级入口**：AI 中心先显示「生成任务树 / 回答与解惑 / 角色扮演」三个入口，点击进入对应板块后，才显示该板块的技能和聊天记录，避免长页滚动。
- **角色扮演新 UI**：角色扮演首页只显示「聊天 / 角色卡」；聊天页按聊天软件样式展示角色联系人和群聊，角色卡页分为 AI 角色卡、我的角色卡、关系和世界观管理；关系地图从角色卡页独立进入，以角色或用户人设为中心展示可缩放拖动的网络图。
- **建议回复模式**：角色扮演聊天页右上角小按钮可切换建议模式，AI 回复后会生成 3 条第一人称回复建议，点击即可发送，也可自由输入。
- **自主性设置**：角色详情里可设置 AI 角色 0~100 的自主性，低自主倾向顺应用户，高自主会提出观点、反驳或引用名家/哲学观点，私聊和群聊均生效。
- **主动发言频率**：行为设置里可单独设置 0~100 的主动发言频率，App 运行期间会按随机阈值触发高自主角色主动说话；群聊中会依次发言，并在输入框上方显示“正在发言”提示，用户发消息即可打断。
- **生成草稿**：Grill-me 生成内容可先保存为草稿，之后选择保存为 AI 角色卡或用户人设卡；角色卡页新增 AI 角色卡 / 我的角色卡 / 草稿分区，并显示数量。
- **世界观导入导出**：世界观管理页支持导出 `.json` 和导入 `.json`/`.txt`，原文件不会被移动或修改。
- **自动关系生成**：关系管理页可用 AI 生成关系建议；创建群聊时可勾选自动生成群内关系，先预览确认再保存。
- **角色聊天列表**：生成角色卡后，角色扮演板块会显示以角色命名的聊天条，点击直接进入私聊；聊天页右上角可查看角色详情。
- **用户人设卡**：可创建多张自己的用户人设卡，自由填写身份、背景和说话方式；开始角色扮演时选择“我的人设卡 + AI角色卡 + 身份/好感度”，同一张用户人设卡可以与多个 AI 角色分别保存不同关系；私聊和群聊都会带上用户人设。
...
- **角色卡生成交接指南**：同目录新对话可读取 [ROLE_CARD_GUIDE.md](ROLE_CARD_GUIDE.md)，按统一格式从文字资料生成可导入的角色卡。
