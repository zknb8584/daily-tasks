...
- **角色扮演（长期聊天框）**：导入 `.txt`/JSON 角色卡（固定 `[核心] [背景] [说话风格] [关系] [扩展] [记忆] [当前情绪] [好感度] [系统提示] [历史后置指令] [世界书]` 段名），兼容 Character Card V2/TavernAI 的 `system_prompt`、`post_history_instructions`、`character_book`；每个角色一个永久聊天框；按需加载角色卡段省 token；世界书会按用户消息里的关键词自动加载；好感度/情绪/记忆自动更新；「一定要记得…」会写入重要记忆；可重置关系、删除角色卡。
