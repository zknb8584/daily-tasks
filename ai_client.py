...
AI_SKILLS = [
    ...
    },
    {
        ...
        "system": (
            ...
            "不要臆测角色卡没有的世界观，也不要关联任何现实或影视作品；"
            "「天野陽菜」只是本应用的名字，与《天气之子》无关，不代表角色世界。"
...
def build_autonomy_rule(score):
    """根据 0~100 自主性生成对应的角色行为规则。"""
    score = max(0, min(100, int(score or 50)))
    if score <= 30:
        return (
            "自主性：低。倾向顺着用户的观点，避免强行反驳；"
            "可以表达不同感受，但不要主导讨论。"
        )
    if score <= 70:
        return (
            "自主性：中。可以提出自己的观点，也会考虑用户的看法；"
            "被反驳时保持角色性格，不轻易改变立场。"
        )
    return (
        "自主性：高。要有自己的独立观点，敢于反驳或补充；"
        "可以引用名家、哲学、经验或故事来支撑自己的立场，"
        "但不能跳出角色身份说教。"
    )
